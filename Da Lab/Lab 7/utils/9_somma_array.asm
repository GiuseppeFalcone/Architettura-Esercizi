...
.data
    # size:   .dword 16
    # array:  .dword 2, 1, 7, 1, 2, 2, 2, 1, 12, 1, 7, 1, 2, 2, 2, 1
   size:   .dword 3
   array:  .dword 2, 1, 7
...
.text
_start:
    la    s1, size    # contiene un indirizzo!
    ...
    # iterative sum
    la    a0, array   # contiene address di array
    ld    a1, 0(s1)   # contiene valore di size
    jal   ra, sumi    # chiamata di funzione
...
sumi:
    ...
    li    t0, 0                # final sum
LOOP_sumi:
    ble   a1, zero, END_sumi   # if a1 <= 0 goto end
    ld    t1, 0(a0)            # first element of the vector
    add   t0, t0, t1           # sum the element
    addi  a1, a1, -1           # decrement the counter
    addi  a0, a0, 8            # move to the next word in the array
    j     LOOP_sumi
